...
const TrainingExperience = () => {
  const trainingData = useQuery(api.training.getAll);
  const loading = trainingData === undefined;
  const error = trainingData instanceof Error ? trainingData : null;
  
  const [filter, setFilter] = useState('All');

  const filtered = filter === 'All' ? trainingData : trainingData?.filter(i => i.type === filter);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {(loading || error || (filtered && filtered.length === 0)) ? (
          <SectionState
            loading={loading}
            error={error}
            empty={!loading && !error && (!filtered || filtered.length === 0)}
          />
        ) : (
          <div className="relative timeline-line pl-12">
            ...
            {filtered?.length === 0 && trainingData?.length > 0 && (
